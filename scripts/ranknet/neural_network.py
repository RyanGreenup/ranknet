...
class three_layer_ranknet_network(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, dtype, dev):
        ...
        self.wi = torch.nn.Parameter(torch.randn(
            input_size, hidden_size, dtype=dtype, requires_grad=True, device=dev))
        self.wo = torch.nn.Parameter(torch.randn(
            hidden_size, output_size, dtype=dtype, requires_grad=True, device=dev))

        self.bi = torch.nn.Parameter(torch.randn(
            hidden_size, dtype=dtype, requires_grad=True, device=dev))
        self.bo = torch.nn.Parameter(torch.randn(
            output_size, dtype=dtype, requires_grad=True, device=dev))

        self.σ = torch.nn.Parameter(torch.randn(
            1, dtype=dtype, requires_grad=True, device=dev))
    ...
    def threshold_train(self, X, y, plot=False):
        ...
        # Set default misclassification as totally wrong.
        rate_best = 1
    ...
    def misclassification_rate(self, X, target, threshold):
        if not self.trainedQ:
            sys.stderr(
                "WARNING: Model has not yet been trained, use the train method")
        rates = []
